...
export default function Sidebar({ activeItem = 'My Account' }) {
  // Navigation structure matching the Figma design
  const navGroups = [
    {
      title: 'General',
      items: [
    ...
    <aside className="fowgate-sidebar">
      ...
      {navGroups.map((group, groupIdx) => (
        <Fragment key={groupIdx}>
            ...
            <ul className="sidebar-menu-list">
              {group.items.map((item, itemIdx) => {
                ...
                return (
                  ...
                  >
                    <a href={`#${item.name.toLowerCase().replace(/\s+/g, '-')}`} className="menu-item-link">
          ...
          {/* Add a divider after every group except the last one */}
          {groupIdx < navGroups.length - 1 && <div className="sidebar-divider"></div>}
        </Fragment>
      ))}
    </aside>
  );
